...
public class AStarImpl {
	 ...
	 *  @param goal
	 *  			- The goal state, represented as a linear array of length 9 forming 3 meta rows.
	 *  @param hueristic
	 *  			- User selected to determine heuristic values; either manhatten distance or number out of place 
	 *  
	 */
	public static void aStarSearch(int[] board, boolean d, int heuristic,int[] goal)
	{
		...
		while (!q.isEmpty()) // while the queue is not empty
		{
			...
			if (!tempNode.getCurState().isGoal())
			{
				...
				ArrayList<StateInformation> tempSuccessors = tempNode.getCurState()
						.genSuccessors();
			...
			// it.
			{
				...
				for (int i = 0; i < loopSize; i++)
				{
					...
					System.out.println("g(n) cost is - "+tempNode.getCost());
					System.out.println("h(n) cost is - "+tempNode.getHCost());
					System.out.println();
					System.out.println();
				}
				System.out.println("The final cost was: " + tempNode.getFCost());
				System.out.println("Total nodes expanded were: "+tempNode.getCost());
				if (d)
				{
